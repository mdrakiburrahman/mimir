...
To see the denormalized `AtomicQuery` that's generated by the `engine.py`, run:

```bash
PGPASSWORD=postgres_password psql -h localhost -p 5432 -U postgres -d pagila << 'EOF'
WITH rentals
     AS (SELECT rental.rental_id,
                ...
                COALESCE(category.NAME, 'Uncategorized') AS dim_rental_category
         FROM   rental
                JOIN inventory
                  ON rental.inventory_id = inventory.inventory_id
                JOIN film_category
                  ON inventory.inventory_id = film_category.category_id
                JOIN category
                  ON film_category.category_id = category.category_id
                LEFT JOIN payment AS payment
                       ON payment.rental_id = rental.rental_id)
SELECT dim_rental_category,
       Count(rental_id) AS movies_rented,
       Sum(amount)      AS rentals_revenue
FROM   rentals
WHERE  dim_rental_category = 'Action'
GROUP  BY 1
EOF
```
...
```text
 dim_rental_category | movies_rented | rentals_revenue 
---------------------+---------------+-----------------
 Action              |           447 |         1336.53
(1 row)
```
